...
export default function PropietariosField() {
    ...
    const onSearch = () => {
        console.log("documento", busqueda.docSearch)
        console.log("telefono", busqueda.phoneSearch)
    }
            ...
            <div className="flex gap-2">
                <div className="flex grow">
                    ...
                    <div className="flex flex-col relative ">
                        ...
                        <p className="absolute p-2 gap-4 -top-10 -left-26  w-36 text-sm rounded-full order-1 transition-all ease-in duration-100 hidden text-blue-400 bg-blue-100 border-2 border-blue-200 opacity-0 peer-hover:opacity-100 peer-hover:block">Buscar Propietario</p>
                    </div>
                </div>
                <div className="flex grow">
                ...
                <div className="flex flex-col relative ">
                    ...
                    <p className="absolute p-2 gap-4 -top-10 -left-26  w-36 text-sm rounded-full order-1 transition-all ease-in duration-100 hidden text-blue-400 bg-blue-100 border-2 border-blue-200 opacity-0 peer-hover:opacity-100 peer-hover:block">Agregar Propietario</p>
                </div>
            </div>

            <div className=" flex flex-col md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {propietario.map((p,idx) => (
                    <div  key={idx} className="flex flex-col w-full  gap px-2 border-2 bg-primary border-blue-400 rounded-lg text-white">
                        <p className="text-blue-900 text-balance">{`${p.name} ${p.last_name}`}</p>
                        <div className="flex justify-between items-end">
                            <div className="flex flex-col items-start">
                                <p><span className="text-blue-900">{`${Documentos.find((d) => d.code == p.document_type)?.name}`}:</span>{p.document}</p>
                                <p><span className="text-blue-900">Tel:</span>{p.phone}</p>
                            </div>
